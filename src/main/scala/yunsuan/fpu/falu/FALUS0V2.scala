...
class FALUS0V2(val totalWidth: Int) extends Module with FloatParams{
  ...
  val fpAIsZero = Mux(isFMA, fmaCtrl.resIsZero, expAIsZero && fracAIsZero)
  val fpBIsZero = expBIsZero && fracBIsZero
  val fpAIsInf = Mux(isFMA, fmaCtrl.resIsInf, expAIsAllOne && fracAIsZero)
  val fpBIsInf = expBIsAllOne && fracBIsZero
  val fpAIsNaN = Mux(isFMA, fmaCtrl.resIsNaN, expAIsAllOne && !fracAIsZero)
  val fpBIsNaN = expBIsAllOne && !fracBIsZero
  val fpAIsSNaN = Mux(isFMA, fmaCtrl.flagsNV, fpAIsNaN && !fracA.head(1))
  val fpBIsSNaN = fpBIsNaN && !fracB.head(1)
...
//  val lzaBGE = LZD(Cat(0.U, Mux(expNE, lzaSeqBG1, lzaSeqBE) | expBMask))
  ...
  val normalSign = Mux(isSub, Mux(AGEB, Mux(absFpAEB, rm === RDN.U, signA), signB), signA)
